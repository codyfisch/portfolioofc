...
//                    todo - /portfolioofc
            ...
            $image = new ImageHandler("/portfolioofc/images");
    ...
    $db = new PDO(DB_INFO, DB_USER, DB_PASS) or die('Could not connect to the database via Update.');

    // Edit an existing entry
    if(!empty($_POST['id']))
    {
        $sql = "UPDATE entries
                SET title=?, subheading=?, image=?, entry=?, url=?
                WHERE id=?
                LIMIT 1";
        $stmt = $db->prepare($sql);
        $stmt->execute(
    ...
    else 
    {
        ...
        $sql = "INSERT INTO entries (page, title, subheading, image, entry, url)
                VALUES (?, ?, ?, ?, ?, ?)";
        $stmt = $db->prepare($sql);
        $stmt->execute(
            array(
...
//                    todo - /portfolioofc
    header('Location: /portfolioofc/'.$page.'/'.$url);
    exit;
}
